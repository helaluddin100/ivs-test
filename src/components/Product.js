...
function Product() {
  ...
  useEffect(() => {
    ...
    fetch(
      `http://admin.infinity-versestudioz.xyz/api/product-details/${productid[1]}`
    )
      .then((res) => res.json())
      .then(
        (result) => {
  ...
  const allInfoSubmit = (e) => {
    ...
    axios
      .post("http://admin.infinity-versestudioz.xyz/api/place-order", formData)
      .then((res) => {
  ...
  const onApprove = (data, actions) => {
    ...
    return actions.order.capture().then(function (details) {
      ...
      axios
        .post(
          "http://admin.infinity-versestudioz.xyz/api/place-order",
          PayPalData
        )
        .then((res) => {
          if (res.data.status === 200) {
